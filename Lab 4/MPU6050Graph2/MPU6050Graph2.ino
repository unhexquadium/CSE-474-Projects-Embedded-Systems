...
#define XA_OFFSET_H      0x06 // User-defined trim values for accelerometerv
...
#define MA_LENGTH_A 2

#define MA_LENGTH_G 10

  int axVals[MA_LENGTH_A]; 
  int ayVals[MA_LENGTH_A];
  int azVals[MA_LENGTH_A];
  int gxVals[MA_LENGTH_G];
  int gyVals[MA_LENGTH_G];
  int gzVals[MA_LENGTH_G];
  int rCount_A;
  int rCount_G;
...
//  tft.fillScreen(ILI9341_WHITE);
  ...
  for (int i = 0; i < MA_LENGTH_A; i++) {
    axVals[i] = 0;
    ayVals[i] = 0;
    azVals[i] = 0;
  }

   for (int i = 0; i < MA_LENGTH_G; i++) {
    gxVals[i] = 0;
    gyVals[i] = 0;
    gzVals[i] = 0;
  }
...
{  
  ...
  if(readByte(MPU6050_ADDRESS, INT_STATUS) & 0x01) {  // check if data ready interrupt
    ...
    rCount_A = rCount_A % MA_LENGTH_A;
    rCount_G = rCount_G % MA_LENGTH_G;
    axVals[rCount_A] = ax;
    ayVals[rCount_A] = ay;
    azVals[rCount_A] = az;
    gxVals[rCount_G] = gx;
    ...
    ax = axTotal / MA_LENGTH_A;
    ay = ayTotal / MA_LENGTH_A;
    az = azTotal / MA_LENGTH_A;
    gx = gxTotal / MA_LENGTH_G;
    gy = gyTotal / MA_LENGTH_G;
    gz = gzTotal / MA_LENGTH_G;
   ...
   // Print temperature in degrees Centigrade      
    ...
    int axGraph = (105 * ax / 2);
    int ayGraph = (105 * ay / 2);
    int azGraph = (105 * az / 2);
    
    tft.fillScreen(ILI9341_WHITE);
    tft.setRotation(1);
    tft.drawLine(0, 110, 320, 110, ILI9341_BLACK);
    tft.drawLine(160, 0, 160, 240, ILI9341_BLACK);

    if (axGraph >= 0) {
      tft.drawRect(20, 110 - axGraph, 20, axGraph, ILI9341_RED);
      tft.fillRect(20, 110 - axGraph, 20, axGraph, ILI9341_RED);
    } else {
      tft.drawRect(20, 110, 20, -axGraph, ILI9341_RED);
      tft.fillRect(20, 110, 20, -axGraph, ILI9341_RED);
    }

    if (ayGraph >= 0) {
      tft.drawRect(60, 110 - ayGraph, 20, ayGraph, ILI9341_BLUE);
      tft.fillRect(60, 110 - ayGraph, 20, ayGraph, ILI9341_BLUE);
    } else {
      tft.drawRect(60, 110, 20, -ayGraph, ILI9341_BLUE);
      tft.fillRect(60, 110, 20, -ayGraph, ILI9341_BLUE);
    }
    
    if (azGraph >= 0) {
      tft.drawRect(100, 110 - azGraph, 20, azGraph, ILI9341_GREEN);
      tft.fillRect(100, 110 - azGraph, 20, azGraph, ILI9341_GREEN);
    } else {
      tft.drawRect(100, 110, 20, -azGraph, ILI9341_GREEN);
      tft.fillRect(100, 110, 20, -azGraph, ILI9341_GREEN);
    }

    tft.setCursor(20, 220); tft.print("ax"); 
    tft.setCursor(60, 220); tft.print("ay"); 
    tft.setCursor(100, 220); tft.print("az"); 

    int gxGraph = (105 * gx / 250);
    int gyGraph = (105 * gy / 250);
    int gzGraph = (105 * gz / 250);
    
    if (gxGraph >= 0) {
      tft.drawRect(200, 110 - gxGraph, 20, gxGraph, ILI9341_RED);
      tft.fillRect(200, 110 - gxGraph, 20, gxGraph, ILI9341_RED);
    } else {
      tft.drawRect(200, 110, 20, -gxGraph, ILI9341_RED);
      tft.fillRect(200, 110, 20, -gxGraph, ILI9341_RED);
    }

    if (gyGraph >= 0) {
      tft.drawRect(240, 110 - gyGraph, 20, gyGraph, ILI9341_BLUE);
      tft.fillRect(240, 110 - gyGraph, 20, gyGraph, ILI9341_BLUE);
    } else {
      tft.drawRect(240, 110, 20, -gyGraph, ILI9341_BLUE);
      tft.fillRect(240, 110, 20, -gyGraph, ILI9341_BLUE);
    }
    
    if (gzGraph >= 0) {
      tft.drawRect(280, 110 - gzGraph, 20, gzGraph, ILI9341_GREEN);
      tft.fillRect(280, 110 - gzGraph, 20, gzGraph, ILI9341_GREEN);
    } else {
      tft.drawRect(280, 110, 20, -gzGraph, ILI9341_GREEN);
      tft.fillRect(280, 110, 20, -gzGraph, ILI9341_GREEN);
    }

    tft.setCursor(200, 220); tft.print("gx"); 
    tft.setCursor(240, 220); tft.print("gy"); 
    tft.setCursor(280, 220); tft.print("gz"); 
